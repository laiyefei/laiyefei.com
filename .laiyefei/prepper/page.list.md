---
layout: default
---

<div>
    <section class="container">
        <div class="content-wrap">
            <div class="content">
                <div class="title csTitleContent"
                    style=" border-bottom: 1px solid black;padding:10px;box-shadow: 0 1px 2px grey">
                    <h3 style="line-height: 1.3;font-weight: bold;">⌘　文章归类</h3>
                </div>
                <div style="padding: 4px;"></div>
                <div style="background:#ffffff91">
                    {% for tag in site.tags %}
                    <div id="{{ tag | first }}" class="list_hook_title" style="padding:40px;display: none;"></div>
                    <div style="box-shadow:1px 1px 1px  #e9f2fb">
                        <h5 id="show_{{ tag | first }}" class="list_hook_title_show"
                            style="padding:10px;font-size: 20px;padding:10px;">
                            {{ tag | first }}
                        </h5>
                        {% for posts in tag %}
                        {% for post in posts %}
                        {% if post.title != null %}
                        <article class="excerpt excerpt-1"><a class="focus" href="#" title="test title"
                                target="_self"><img class="thumb"
                                    data-original="/assets/img/biz/title/{{ post.date | date: "%Y-%m-%d" }}.jpg"
                                    src="/assets/img/biz/title/{{ post.date | date: "%Y-%m-%d" }}.jpg" alt="test alt"
                                    style="display: inline;"></a>
                            <header><a class="cat" href="javascript:;" title="{{ post.categories }}">{{ post.categories
                                    }}<i></i></a>
                                <h2><a href="{{ post.url | prepend: site.baseurl | prepend: site.url }}"
                                        title="{{post.title}}" target="_self">{{post.title}}</a></h2>
                            </header>
                            <p class="meta">
                                <time class="time"><i class="glyphicon glyphicon-time"></i> {{ post.date | date:
                                    "%Y-%m-%d" }}</time>
                                <span class="views"><i class="glyphicon glyphicon-eye-open"></i> 217</span> <a
                                    class="comment" href="##comment" title="评论" target="_self"><i
                                        class="glyphicon glyphicon-comment"></i> 4</a>
                            </p>
                            <p class="note">
                                {{post.note}}
                            </p>
                        </article>
                        <div style="padding: 5px;"></div>
                        {% endif %}
                        {% endfor %}
                        {% endfor %}
                        <div style="padding: 3px;"></div>
                    </div>
                    <div style="padding: 15px;"></div>
                    {% endfor %}
                    <div style="padding: 1024px 0"></div>
                </div>
            </div>
        </div>
        <aside class="sidebar">
            <div class="fixed">
                <div class="widget widget_hot">
                    <div>{% include tag.wall.md %}</div>
                </div>
            </div>
        </aside>
    </section>
</div>